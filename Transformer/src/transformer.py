...
class Transformer():
    ...
    def __init__(self,train_length):
        # this defines how long the training data is
        self.train_length=train_length


    def encoder(self):

        # this section is for the five 2-D inputs for the five letters
        ...
        """Multi-headed attention is used...hence each letter vector is replicated five times to mimic five heads
        of the multi-headed attention procedure
        """

        #First vector a1 is replicated 5 times using 5 2x2 matrices that are learnable
        ...
        #this is the attention step where each vector is compared against other vectors before being softmaxed
        ...
        #the comparison inputs are processed through a softmax layer
        ...
        #this is where the softmax results are multiplied with corresponding vectors
        #there are 5 outputs for 5 vectors
        ...
        #Here results from the five heads are concatenated before being processed by the feedforward network
        ...
        #this is some boilerplate code to prepare the outputs self.ffnnx for input to the decoder
        #first the 10 items of each self.ffnnx are unstacked and then pairs are stacked into 5 pairs 
        # to be entered as encoder output into the decoder
    ...
    def decoder(self):
        ...
        #this variable is needed to initiate the process of plotting points
        self.first = tf.Variable([[10], [10]], shape=(2, 1), dtype=tf.float32)

        # shape=(2,5)
        # encoder outputs are prepared here
        ...
        #this is essentially the query vector that comes out of the decoder self-attention process
        prenn1 = self.first

        # the output of the decoder self-attention is multiplied here with the corresponding encoder output
        #to generate weights for encoder-decoder attention before being softmaxed
        z1 = tf.compat.v1.matmul(encoder_output1, prenn1)

        #softmaxing the weights z1
        ...
        #preparing the input for the position wise feedforward network that produces the next 2-D point
        prenn21 = tf.compat.v1.matmul(softmaxed21, encoder_output1) + prenn1  # shape=(1,2)

        #FFNN produces next 2-D point...these layers are fixed for all positions
        ffnn1 = Dense(2, input_shape=(2,), activation='relu', name='l0')(prenn21)  # shape=(1,2)
        ffnn1 = Dense(2, name='m0')(ffnn1)  # shape=(1,2)

        # The first next point is added to the output list
        output_list.append(tf.reshape(ffnn1, shape=(2, 1)))

        for j in range(1, 24):

            #this contain the weights of self-attention to be softmaxed and mat-multiplied later
            x = []
            #this contains vectors that are to be multiplied with softmaxed weights to produce query
            # vector for encoder-decoder attention
            y = []

            for k in range(0, 24):

                # This step is the masked multiheaded attention for decoder self-attention

                if k < j:
                    #if position of key vectors are less than that of the query vector then
                    #they are compared
                    x.append(tf.compat.v1.reduce_sum(tf.compat.v1.multiply(output_list[k], output_list[j])))
                    y.append(output_list[k])
                else:
                    # otherwise they play no role
                    x.append(0)
                    y.append([[0], [0]])
            ...
            #same procedure for attention as in encoder...first softmax the weights and then matmul with the vectors
            ...
            #since the feedforward networks are to be treated positionwise, we need 24 different networks,
            # one for each position...each named according to position
            if j == 1:
            ...
            #output list grows as the process continues
            output_list.append(tf.reshape(ffnn1, shape=(2, 1)))

        #the first input was just to start a uniform initialization and is hence popped
        output_list.pop(0)

        #preparing the final points for this encoderinput
        self.decoded1 = tf.stack(output_list)
        self.decoded1 = tf.reshape(self.decoded1, shape=(2, -1))


        # fourth encoder output
        output_list = [self.first]
        ...
        #hence 5 decoder outputs of length (2,24) are prepared for the 5 encoder outputs
    ...
    def train(self):
        ...
        #this is the output set of points for each letter
        y_tensor = tf.compat.v1.placeholder(shape=[2, 24], dtype=tf.float32)
        learning_rate = 0.00000001

        #for each letter position a different loss is to be optimized
        ...
        #you van change number of epochs here
        num_epochs = 1
        ...
        with tf.compat.v1.Session() as tfs:
            ...
            for num in range(num_epochs):
                ...
                for i in range(len(train_chars)):
                    ...
                    #5 cases for 5 letter positions
    ...
    def test(self, string):
        ...
        #points are saved in a file 'points.csv'
        predictions = np.reshape(predictions, (2, -1))
        predictions = pd.DataFrame(predictions)
        predictions.to_csv('points.csv', index=False)
